...
/* K ničemu - blokování při špatném vstupním řetězci pro převod na datum
    public static LocalDate prevodStrToDate (String sDatum) {
        LocalDate datum = LocalDate.now();
        datum = LocalDate.parse(sDatum);

        try {
             datum = LocalDate.parse(sDatum);
             throw new PlantException ("Problém s převodem řetězce na datum !"+sDatum);
        }
        catch (PlantException | DateTimeParseException e) {
            System.out.println("Nesprávně zadán řetězec se vstupním datumem " + sDatum + " - doplněno AKTUÁLNÍ DATUM " + datum);
        }
    ...
    PlantList result = new PlantList();
        ...
        try (Scanner scanner = new Scanner(new File(filename))) {
            while (scanner.hasNextLine()) {
                ...
                System.out.println(line);      // ----- pomocný výpis na obrazovku při ladění aplikace
...
//                Toto nefungovalo, totálně se "zakusoval" běh aplikace ......
//
...
//
                // Tímhle jsem si vůbec nepomohl ....
//                LocalDate planted = prevodStrToDate (plantedAsText);
//                LocalDate watering = prevodStrToDate (wateringAsText);
    ...
    public Plant getPlantFromIndex (int indexOfPlant) throws PlantException {
        // Ošetření chybně zadaného indexu odkazujícího mimo pole
        if (indexOfPlant < 0) {
            throw new PlantException("Hodnota indexu " + indexOfPlant + " odkazujícího na prvek pole nesmí být záporná !");
        }
        if (indexOfPlant >= plantList.size()) {
            throw new PlantException("Hodnota indexu " + indexOfPlant + " odkazujícího na prvek pole nesmí být větší jak počet prvků pole minus jedna !");
        }
